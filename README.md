...
>   - 圆心角之和sum<2π，线段没法首尾相连的在外接圆上，半径变小
>   - 圆心角之和sum>2π，半应变大
...
如果需要求最长路径长度，则可以把所有边的边权乘以-1，令其变为相反数，然后使用 Bellman-Ford 算法或 SPFA 算法求最短路径长度

e[r] 和 l[r] 分别表示活动 a_r 的最早开始时间和最迟开始时间，于是，通过拓扑排序和逆拓扑排序求出这两个数组后，就可以通过判断 e[r]==l[r] 是否成立来确定活动 r 是否是关键活动

### 动态规划

#### 数塔问题

**现在要从第一层走到第n层，每次只能走向下一层连续的两个数字中的一个，问：最后将路径上所有数字相加后得到的和最大是多少？**

![数塔问题](suanfa_md_files/%E6%95%B0%E5%A1%94%E9%97%AE%E9%A2%98_20201114191844.png?v=1&type=image&token=V1:6qUL3rdhFjFhlv8yJtkKx_ktifgvoeh4iX_Lb7nlloU)

> f[i][j]存放第i层第j个数字，dp[i][j]表示第i行第j个数字粗发的到达最底层的所有路径中能得到的最大和，
> 则状态转移方程: $$dp[i][j] = max(dp[i+1][j], dp[i+1][j+1]) + f[i][j]$$

#### 最大连续子序列和

**给定一个数字序列A_1,A_2,…,A_n，求i,j (1≤i≤j≤n)，使得A_i+,…,A_j最大，输出这个最大和。**
>令状态dp[i]表示以A[i]作为末尾的连续序列的最大和，
>则状态转移方程: $$dp[i]=max\{A[i], dp[i-1] + A[i]\}$$
...
> 则状态转移方程: $$dp[i]=max\{1,dp[j]+1\}$$ $$(j=1,2,…i-1 \land A[j] < A[i])$$
...
dp[i][j] =
\begin{cases}
dp[i-1][j-1]+1, & \text{A[i] == B[j]} \\
max\{dp[i-1][j],dp[i][j-1]\}, & \text{A[i] != B[j]}
\end{cases}
$$
...
> 按此dp[i]可以正向求字典序小的路径序列
> 也可以令dp[i]表示以i号顶点为终点能获得的最长路径长度
...
> 令dp[i]表示从i号顶点出发到达终点T能获得的最长路径长度
> 边界处理区别第一种情况:
>
> - 初始化dp数组为一个负的大数，来保证没有出度的顶点“无法到达终点”的含义的得以表达
>
> - 设置一个vis数组表示顶点是否已经被计算，因为第一种情况的dp数组初始值为0，这个初始值为-INF
...
> 则状态转移方程:$$dp[i][v] = max\{dp[i-1][v],dp[i-1][v-w[i]]+c[i]\}$$
> 令dp[v]表示此时(0≤v≤V)恰好装入容量为v的背包中所能获得的最大价值。(v的枚举顺序变为从右往左)
> 则状态转移方程:$$dp[v] = max\{dp[v],dp[v-w[i]]+c[i]\}$$

##### 完全背包

**有n种物品，每种物品的单价重量为w[i]，价值为c[i]。现有一个容量为V的背包，问如何选取物品放入背包，使得背包内物品的总价值最大。其中每件物品都是无穷件。**

> 令dp[i][v]表示前i件物品(1≤i≤n,0≤v≤V)恰好装入容量为v的背包中所能获得的最大价值。
> 则状态转移方程:$$dp[i][v] = max\{dp[i-1][v],dp[i][v-w[i]]+c[i]\}$$
> 令dp[v]表示此时(0≤v≤V)恰好装入容量为v的背包中所能获得的最大价值。(v的枚举顺序变为从左往右)
> 则状态转移方程:$$dp[v] = max\{dp[v],dp[v-w[i]]+c[i]\}$$
